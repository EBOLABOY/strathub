...
export function createRealFactory(prisma: PrismaClient): ExecutorFactory {
    const cache = new Map<string, { executor: TradingExecutor; accountUpdatedAtMs: number }>();

    return async (exchangeAccountId: string): Promise<ExecutorContext> => {
        // Always read account to detect credential updates (updatedAt changes on edit).
        ...
        const accountUpdatedAtMs = account.updatedAt.getTime();
        const cached = cache.get(exchangeAccountId);
        if (cached && cached.accountUpdatedAtMs === accountUpdatedAtMs) {
            return { executor: cached.executor };
        }

        // Decrypt Credentials (supports both encrypted and plaintext)
