...
- **V1 不是“可实盘跑起来的网格机器人”**：真实交易所 Provider、Worker Loop、以及策略执行（触发检查→下单→撤单）不在 V1。
...
| 实时推送（SSE/WebSocket） | V1 不做推送，先靠轮询/日志/HTTP seam | V2 |
| 多交易所同时运行 | 先跑单交易所 | V2 |
| 实盘交易（真实下单/余额/仓位） | V1 仅有 provider seam 与模拟器验证 | V2 |
| 可观测性（metrics/告警面板） | V1 测试锁语义，不承诺指标实现 | V2 |
